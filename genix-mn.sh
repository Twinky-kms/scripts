...
if [[ $DOSETUP =~ "y" ]]; then
    ...
    rm -r /root/dash-fork/genix-fork/
...
for i in $(seq 1 1 $MNCOUNT); do
    ...
    echo "pigeond -daemon -conf=$CONF_DIR/genix.conf -datadir=$CONF_DIR "'$*' >>~/bin/genixd_$ALIAS.sh
    echo '#!/bin/bash' >~/bin/pigeon-cli_$ALIAS.sh
    echo "pigeon-cli -conf=$CONF_DIR/genix.conf -datadir=$CONF_DIR "'$*' >>~/bin/genix-cli_$ALIAS.sh
    echo '#!/bin/bash' >~/bin/pigeon-tx_$ALIAS.sh
    echo "pigeon-tx -conf=$CONF_DIR/genix.conf -datadir=$CONF_DIR "'$*' >>~/bin/genix-tx_$ALIAS.sh
    ...
    echo "addnode=45.76.113.77" >> genix.conf_TEMP
